...
private:									//elementos privados: atributos
	...
	int numDatosAct;						//numero actual de elementos
											//metodos privados
	int getPosPadre(int i); 				// PADRE DETERMINA LA POSICION //metodo que devuelve la posicion de padre de i
	int getPosHiIzq(int i); 				// POSICION DEL HIJO IZQUIEROD //metodo que devuelve la posicion de hijo izquierdo de i
	int getPosHiDer(int i); 				// POSICION DEL HIJO DERECHO   //metodo que devuelve la posicion de hijo derecho de i
...
public:										//metodos publicos
	...
	Clave BuscarMinimo(); 					//retorna el elemento menor del montículo// busca la raiz 
	Clave EliminarMinimo(); 				//elimina el primer elemento// dovolver el valor mínimo
...
Monticulo::Monticulo(int n){					//constructor conociendo cantidad de elementos a procesar
	...
	setNumDatosAct(0);							//hasta el momento no existen valores en monticulo: inicializa contador
	val = new Clave[getNumDatosMax()];			// se crea dinamicamente//inicializa el vector de valores con capacidad para cantidad maxima de elmemtos
}
...
void Monticulo::Insertar(Clave v){				//inserta nuevo dato en monticulo
	...
	if (!Lleno()) {								//monticulo no lleno => insertar
		...
		flotar(getNumDatosAct());				// se ubica donde le//desplaza elementos para conservar relacion entre elmentos
