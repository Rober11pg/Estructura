// ArbolBinario.h
// Clase que contiene metodos de gestion de un ABB
...
private:               // atributos
    ...
    { // metodo privado para imprimir espacios
        for (int i = 1; i <= c; i++)
    ...
    {                     // metodo privado para ingresar AB por nodos
    ...
    { // implementa Recorrido de un árbol binario en preorden
        // raiz, izquierdo, derecho
        if (r != NULL)
        {
            cout << " " << r->getDato();
            // dato, punt izquierdo, punt derecho
            preorden(r->getIzq());
            preorden(r->getDer());
        }
    }
    ...
    { // implementa Recorrido de un árbol binario en inorden
        if (r != NULL)
        {
            inorden(r->getIzq());
            cout << " " << r->getDato();
    ...
    { // implementa Recorrido de un árbol binario en postorden
        if (r != NULL)
        {
            postorden(r->getIzq());
            postorden(r->getDer());
    ...
    //==================================================================================================
// Implementa casi el arbol binario de búsqueda
    NodoBinario *insertar(NodoBinario *rSub, Tipo v)
    { // implementa la inserción de un nodo en AB
        if (rSub == NULL)
        {                              // no hay nodos en arbol
            rSub = new NodoBinario(v); // crea nuevo nodo y lo apunta desde rsub
        }
        else
        { // si hay nodos en el arbol
            if (v < rSub->getDato())
            {                                                  // valor a insertar es menor que valor del nodo actual
                NodoBinario *iz = insertar(rSub->getIzq(), v); // tratar de insertar a la izq del nodo actual
                rSub->setIzq(iz);                              // actualiza apuntador
            }
            else
            { // valor a insertar no es menor que valor nodo actual
                if (v > rSub->getDato())
                {                                                  // valor a insertar es mayor que valor nodo actual
                    NodoBinario *dr = insertar(rSub->getDer(), v); // tratar de insertar a derecha del nodo actual
                    rSub->setDer(dr);                              // actualiza apuntador
                }
                else
                {                // valor a insertar ya existe en arbol
    ...
    NodoBinario *eliminar(NodoBinario *rSub, Tipo v)
    { // implementa la eliminacion de un nodo del AB
        if (rSub == NULL)
        {                // arbol no tiene nodos
            return NULL; // No se ha encontrado el nodo con el valor ingresado
        }
        else
        { // arbol si tiene nodos
            if (v < rSub->getDato())
            {                                                  // valor buscado es menor que valor de nodo actual
            ...
            { // valor buscado no es menor que valor nodo actual
                if (v > rSub->getDato())
                {                                                  // valor buscado es mayor que valor del nodo actual
                ...
                else // lo econtré de aquí procedo a eliminar 
                {                          // Nodo encontrado con valor buscado
                    NodoBinario *q = rSub; // nodo a quitar del árbol
                    if (q->getIzq() == NULL)
                    {                       // ramal izquierdo de este nodo es NULL
                        rSub = q->getDer(); // se toma ramal derecho
                    }
                    else
                    { // si hay nodo apuntado en ramal izquierdo
                        if (q->getDer() == NULL)
                        {                       // No hay nodo apuntado por ramal derecho
    ...
    NodoBinario *reemplazar(NodoBinario *act)
    { // sustituye un nodo por otro en AB (parte de eliminación)
        NodoBinario *a, *p;
        p = act; // guardar lo de rsub 
        a = act->getIzq(); // rama de nodos menores
        while (a->getDer() != NULL)
        {                    // mientras haya nodos en ramal derecho
    ...
    NodoBinario *buscar(NodoBinario *rSub, Tipo v)
    { // implementa búsqueda de un nodo en el AB
        if (rSub == NULL)
        {                // no existen nodos
        ...
        { // si hay nodos
            if (v == rSub->getDato()) // nodo raiz 
            {                // valor buscado esta en este nodo
                return rSub; // devuelve apuntador a este nodo
            }
            else
            { // valor buscado no esta en este nodo
                if (v < rSub->getDato())
                {                                     // valor buscado es menor que valor del nodo
                    return buscar(rSub->getIzq(), v); // repite busqueda en subarbol izquierdo
                }
                else
                {                                     // valor buscado no es menor que valor del nodo
...
public:             // metodos publicos
    ...
    int SumarNodosRama(NodoBinario *r);
    void MayorNivel(NodoBinario *r);
};
...
{ // metodo que devuelve el atributo
    return raiz;
}
...
{                               // metodo que determina si el AB carece de nodos
...
{                         // metodo que permite lectura de un AB
...
{ // metodo que imprime un AB
    if (p != NULL)
    {
        cout << endl << "> " << p->getDato();
        imprimirABSimple(p->getIzq()); // llamados recursivos que impiren todos los nodos del subarbol
...
{ // visualiza el abb 90@ a la izquierda
    if (p == NULL)
...
{ // determina la cantidad de nodos que posee un arbol
    if (r == NULL)
    {
        return 0;
    }
    else
    {
        ...
        // Importante: En este caso, necesitamos contar el nodo actual (que es el nodo raíz del subárbol actual).
    }
}
...
{ // determina la altura de un arbol
    ...
    else
    {
        ...
        { // determina si altura de subarbol izq es mayor que la de subarbol der
            return alturaIz + 1;
        }
        else
        {
...
void ArbolBinario::verHojas(NodoBinario *r)
{ // metodo de recorrido de nodos hoja							//visualiza las hojas de un AB
    if (r != NULL)
    { // nodo recibido no es nulo
        if ((r->getIzq() == NULL) && (r->getDer() == NULL))
        { // no tiene nodos hijo => hoja
            cout << endl
                 << "> " << r->getDato(); // visualiza dato contenido
        }
        else
        {                          // si tiene nodos hijo => nodo rama
...
int ArbolBinario::SumarNodosRama(NodoBinario* r)
{
    if (r == NULL || (r->getIzq() == NULL && r->getDer() == NULL))
    {
        return 0; // Si el nodo es nulo o una hoja, retorna 0
    }
    
    int suma = r->getDato(); // Suma el valor del nodo actual
    
    if (r->getIzq() != NULL)
        suma += SumarNodosRama(r->getIzq()); // Suma los nodos de la rama izquierda

    if (r->getDer() != NULL)
        suma += SumarNodosRama(r->getDer()); // Suma los nodos de la rama derecha
...
{                                    // permite insertar un nuevo nodo con valor v en un arbol binario
...
{                                    // permite eliminar un nodo con valor v de un arbol binario
...
{ // determina si el arbol es lleno o no
    ...
    { // si hay nodos
        if (ArbolBinario::calcularAltura(r->getIzq()) != ArbolBinario::calcularAltura(r->getDer()))
