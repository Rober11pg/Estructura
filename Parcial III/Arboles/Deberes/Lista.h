...
Lista::Lista(){				//metodo constructor
	setPrimero(NULL);		//inicializa comienzo de la lista
}

void Lista::setPrimero(Nodo* v){	//metodo que actualiza el estado del atr
	primero = v;					//guarda parametro en atributo
}

Nodo* Lista::getPrimero(){		//devuelve el estado del atr
	return primero;				//retorna estado del atr
}

bool Lista::esVacia(){			//metodo que determina si la lista esta vacia
	return (getPrimero() == NULL);
}

void Lista::vaciar(){			//metodo que elimina todos los nodos de la lista
	setPrimero(NULL);			//actualiza primero a null
}

void Lista::insertarAlInicio(Tipo v){	//metodo que permite insertar nuevos nodos al inicio de la lista
...
void Lista::insertarAlFinal(Tipo v){	//metodo que permite insertar nuevos nodos al final de la lista
...
void Lista::insertarEnOrden(Tipo v){	//metodo que permite insertar nuevos nodos ordenadamente en la lista
	...
	if (esVacia()){						//si la lista no contiene nodos aun
		...
		while ((act != NULL) && (act->getDato() < v)) {			//recorrido mientras el nodo apuntado no apunta a null
			ant = act;					
...
Nodo* Lista::buscarElemento(Tipo v){			//metodo que permite devolver la direccion del nodo que contiene el valor v
	Nodo* act;							//apuntador temporal al nodo actual
	
	act = getPrimero();					//iniciamos con la direccion apuntada por inicio de lista
	if (!esVacia()){			//si la lista contiene nodos
		while ((act != NULL) && (act->getDato() != v)){	//recorrido mientras el nodo apuntado no apunta a null
			act = act->getPunt();		//actualizar apuntador de recorrido
		}
	}
...
bool Lista::eliminarNodo(Tipo v){		//metodo que permite eliminar el nodo que contiene el valor v
...
Nodo* Lista::ultimo(){			//metodo que devuelve la direccion del ultimo nodo de la lista
	Nodo* act;					//apuntador temporal al nodo actual
	
	act = getPrimero();		//iniciamos con la direccion apuntada por inicio de lista
	if (!esVacia()){			//si la lista contiene nodos
		while(act->getPunt() != NULL){	//recorrido mientras el nodo apuntado no apunta a null
			act = act->getPunt();		//actualizar apuntador de recorrido
		}
	}
	return act;							//devuelve direccion del nodo apuntado
...
bool Lista::eliminarAlInicio() {
    if (esVacia()) {
        return false; // Si la lista está vacía, no se puede eliminar nada
    }

    Tipo valor = getPrimero()->getDato(); // Obtiene el valor del primer nodo
    return eliminarNodo(valor); // Llama al método eliminarNodo() para eliminar el nodo con ese valor
}
