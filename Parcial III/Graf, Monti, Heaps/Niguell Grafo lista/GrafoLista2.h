...
void GrafoLista::setVertice(string a, Vertice v) {			//modifica los atributos de un vertice, conociendo su numero
...
void GrafoLista::setArco(int va, int vb){					//actualiza valor de arco recibiendo numeros de vertices en grafo no valorados
	if ((va >= 0) && (vb >= 0) && (va < getNumVerts()) && (vb < getNumVerts())){	//comprueba existencia de los dos vertices
		arcos[va] -> insertarAlFinal(vb);					//añade un nuevo vertice en la lista de este elemento del vector
	}
}

void GrafoLista::setArco(int va, int vb, int v){			//actualiza valor de arco recibiendo numeros de vertices y factor de peso
	if ((va >= 0) && (vb >= 0) && (va < getNumVerts()) && (vb < getNumVerts())){	//comprueba existencia de los dos vertices
		arcos[va] -> insertarAlFinal(vb, v);					//añade un nuevo vertice en la lista de este elemento del vector
	}
}
...
Vertice GrafoLista::getVertice(string a) {					//retorna todos los atributos del vértice si está almacenado en el grafo, conociendo su nombre
	int va;													//variable para contener numero de vertice
	va = getNumVertice(a);										//calcula numero de vertice del primer nombre recibido
...
bool GrafoLista::getArco(string a, string b) {				//retorna el valor del arco almacenado en la matriz de adyacencia para dos vértices que recibe como parámetro por nombre
	int va, vb;												//variables para contener numeros de los vertices recibidos por nombre
...
int GrafoLista::getNumVertice(string v) {					//determina el numero de un vertice dentro del grafo, conociendo su nombre
...
void GrafoLista::nuevoVertice(string v) {					//crea un nuevo vertice recibiendo su nombre
