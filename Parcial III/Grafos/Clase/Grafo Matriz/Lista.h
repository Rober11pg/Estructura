...
public:							   // definicion de metodos publicos
	...
	bool existeModa();
	int obtenerTamanio();
	int obtenerElementoEnPosicion(int pos);
	int sumaNodosPares();
	int sumaNodosImpares();
};
...
int Lista::obtenerElementoEnPosicion(int pos)
{
	Nodo *actual = getPrimero();
	int contador = 0;
	...
	return -1; // Si la posición es inválida, se devuelve -1 o un valor adecuado según tu implementación
}

int Lista::sumaNodosPares()
{
	int suma = 0;
	Nodo *actual = getPrimero();
