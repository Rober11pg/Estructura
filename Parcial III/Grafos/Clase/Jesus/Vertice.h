// vertice.h
// Implementa la definicion de los v�rtices de un grafo
...
class Vertice {						//implementaci�n de la clase
private:							//atributos
...
public:								//metodos publicos
	...
	Vertice(string no);		//constructor conociendo valor a almacenar en vertice y pone el n�mero de v�rtice e -1
	Vertice(string no, int nu);//constructor conociendo valor a almacenar en v�rtice y numero que le corresponde
	...
	bool igual(string no);		//determina si nombre del vertice es igual al par�metro
...
Vertice::Vertice(string no) {			//constructor conociendo valor a almacenar en vertice y pone el n�mero de v�rtice e -1
	setNombre(no);							//actualiza nombre
	setNumero(-1);							//actualiza numero
}

Vertice::Vertice(string no, int nu) {	//constructor conociendo valor a almacenar en v�rtice y numero que le corresponde
	setNombre(no);							//actualiza nombre
...
bool Vertice::igual(string no) {		//metodo que determina si nombre del vertice es igual al parametro
	return (getNombre() == no);				//calcula valor booleano producto de comparaci�n
}
