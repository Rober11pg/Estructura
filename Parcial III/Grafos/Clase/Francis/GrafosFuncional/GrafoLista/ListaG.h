...
public:										//elementos publicos: métodos
	...
	void insertarAlFinal(Tipo v);			//metodo que inserta un elemento al final de la lista
...
ListaG::ListaG(){							//constructor
	setPrimero(nullptr);						//inicializar el apuntador de inicio de la lista
}
...
bool ListaG::esVacia(){						//metodo devuelve true si la lista esta vacia
	return primero == nullptr;					//devuelve true si primero apunta a null
}
...
void ListaG::insertarAlFinal(Tipo v){		//metodo que inserta un elemento al final de la lista
	NodoG* nuevo;							//declaracion de un nuevo nodo
	NodoG* ultimo = getPrimero();			//apuntador al ultimo nodo de la lista
	
	nuevo = new NodoG(v);					//llamar metodo constructor de nodo pasandole el valor recibido como parametro
	if (ultimo == nullptr){					//lista esta vacia
...
void ListaG::insertarAlFinal(Tipo v, int q){//metodo que inserta un elemento al final de la lista
	NodoG* nuevo;							//declaracion de un nuevo nodo
	NodoG* ultimo = getPrimero();			//apuntador al ultimo nodo de la lista
...
NodoG* ListaG::ultimoValorDeLista(){		//metodo que devuelve la direccion del ultimo nodo de la lista
	...
	while (actual->getPunt() != nullptr){		//repetir el ciclo mientras el nodo apuntado por actual no apunta a NULL
...
NodoG* ListaG::buscarValorEnLista(Tipo v){	//metodo que devuelve la direccion del nodo que contiene el valor v
	...
	actual = getPrimero();					//copiar la direccion del primer nodo de la lista
	while (actual != nullptr){					//repetir el ciclo mientras el nodo apuntado por actual no apunta a NULL
	...
	return nullptr;							//si el valor no se encuentra, devolver null
}
...
void ListaG::eliminarNodoDeLista(Tipo v){	//metodo que elimina el nodo que contiene el valor v de lista
	...
	actual = getPrimero();					//copiar la direccion del primer nodo de la lista
	ant = nullptr;								//por defecto, no existe un nodo antes del inicio de la lista
	enc = false;							//por defecto, el valor v no se encientra en la lista	

	while ((actual != nullptr) && (!enc)){		//repetir el ciclo mientras el nodo apuntado por actual no apunta a NULL y no lo encuentre aun
	...
	if (actual != nullptr){					//si el nodo actual no esta al final de la lista
...
void ListaG::insertarEnOrden(Tipo v){		//metodo que inserta ordenadamente un valor dentro de una lista
	...
	nuevo = new NodoG(v);					//llamar metodo constructor de nodo pasandole el valor recibido como parametro
	if (ult == nullptr){						//lista vacia => nuevo nodo será el primero
		...
		if (v < ult -> getDato()){			//nuevo valor es menor que lo almacenado en el nodo apuntado desde ult
			...
			while((ult -> getPunt() != nullptr) && (v > ult->getDato())){
